...
// GetContextualDescription returns a human-readable description of the step context
func (sc *StepContext) GetContextualDescription() string {
	description := ""

	if sc.LoopContext != nil {
		switch sc.LoopContext.Type {
		case "for":
			description += "in for loop "
			if sc.LoopContext.Item != nil {
				description += "with item '" + fmt.Sprintf("%v", sc.LoopContext.Item) + "' "
			}
			description += "at iteration " + fmt.Sprintf("%d", sc.LoopContext.Iteration)
		case "while":
			description += "in while loop at iteration " + fmt.Sprintf("%d", sc.LoopContext.Iteration)
		}
	}
	...
	if sc.Conditions != nil {
		if sc.Conditions.IfCondition != "" {
			...
			description += "with if condition '" + sc.Conditions.IfCondition + "'"
		}
	}
